...
%       > r : parameter controlling the window size
%             - scalar, cubic window dimensions
...
% version extension 2018.12.18 (r is now a 3x1 vector)
if length(r)==1
    tmp = r;   r = zeros(3,1);   r(:) = tmp;
end

% check the consistency between the number of outputs and str_out
assert(nargout==length(str_out),['number of output arguments must be ',...
                                 'consistent with that defined in the ',...
                                 'string vector "str_out", see help ',...
                                 'featureParam_bloc'])
